...
test('persists weekly checklist on reload', async ({ page }) => {
  ...
  await expect(page.getByTestId('weekly-progress')).toContainText('Completed 1 of 8 tasks')
...
test('runs overall skills mode and can reset program', async ({ page }) => {
  ...
  await expect(page.getByTestId('weekly-progress')).toContainText('Completed 0 of 8 tasks')
})
